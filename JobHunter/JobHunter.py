...
# Fall 2019 CNA 330
# Jasmine Holman, jjholman@student.rtc.edu, Robin Cunanan helped me complete this project in class group 
#
...
def check_if_job_exists(cursor, jobdetails):
    job_id = jobdetails['id']
    query = "SELECT * FROM jobs WHERE Job_id = \"%s\"" % job_id  # Help from Robin Cunanan
    return query_sql(cursor, query)

# Deletes job
def delete_job(cursor, jobdetails):
    job_id = jobdetails['id']
    query = "DELETE FROM jobs WHERE Job_id = \"%s\"" % job_id  # Help from Robin Cunanan & https://www.tutorialspoint.com/mysql/mysql-delete-query.htm
    return query_sql(cursor, query)
...
def add_or_delete_job(jobpage, cursor):
    ...
    for jobdetails in jobpage:  # EXTRACTS EACH JOB FROM THE JOB LIST
        ...
        if is_job_found:  # Help from Robin Cunanan
            # DELETE JOB
